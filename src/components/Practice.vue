...
Difficulty
    ...
    />
<q-toggle v-model="kashmiri" label="Kashmiri Letters" />
    ...
    <span :key="resetV">
      <q-card v-for="(i,index) in randomList" :key="i" inline class="cards q-ma-sm"
       :text-color="colors(index)">
        <q-card-title>
          <q-input placeholder="answer" ref="'q' + i" v-model="answers['q' + index]" readonly></q-input>
        </q-card-title>
        ...
        <q-card-actions align="around" horizontal v-show="selections">
          <q-btn-toggle
                ...
                text-color="black"
                :options="[
                {label: script === 'devanagari' ? compounds[randomOptions[index][0]] : convertDR(compounds[randomOptions[index][0]]), value: compounds[randomOptions[index][0]]},
                {label: script === 'devanagari' ? compounds[randomOptions[index][1]] : convertDR(compounds[randomOptions[index][1]]), value: compounds[randomOptions[index][1]]},
                {label: script === 'devanagari' ? compounds[randomOptions[index][2]] : convertDR(compounds[randomOptions[index][2]]), value: compounds[randomOptions[index][2]]}
                ]"
              flat
              dense
              size="11px"
